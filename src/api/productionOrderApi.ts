...
export interface BusinessApiResponse {
  ...
  export const createBusiness = async (businessData: BusinessCreateRequest): Promise<BusinessApiResponse> => {
    try {
      ...
      return {
        success: true,
        data: result,
        message: '생산지시 정보가 성공적으로 등록되었습니다.'
      };
    } catch (error) {
      console.error('생산지시 정보 등록 실패:', error);
      return {
        success: false,
        error: error instanceof Error ? error.message : '생산지시 정보 등록에 실패했습니다.'
      };
    }
  };
  
  // 생산지시 정보 수정 (PATCH /work-orders/:id)
  export const updateBusiness = async (businessData: BusinessUpdateRequest): Promise<BusinessApiResponse> => {
    try {
      ...
      return {
        success: true,
        data: result,
        message: '생산지시 정보가 성공적으로 수정되었습니다.'
      };
    } catch (error) {
      console.error('생산지시 정보 수정 실패:', error);
      return {
        success: false,
        error: error instanceof Error ? error.message : '생산지시 정보 수정에 실패했습니다.'
      };
    }
  };
  
  // 생산지시 정보 삭제 (DELETE /work-orders/:id)
  export const deleteBusiness = async (id: number): Promise<BusinessApiResponse> => {
    try {
      ...
      return {
        success: true,
        message: '생산지시 정보가 성공적으로 삭제되었습니다.'
      };
    } catch (error) {
      console.error('생산지시 정보 삭제 실패:', error);
      return {
        success: false,
        error: error instanceof Error ? error.message : '생산지시 정보 삭제에 실패했습니다.'
      };
    }
  };
  
  // 생산지시 정보 조회 (GET /work-orders)
  export const getBusinesses = async (): Promise<BusinessApiResponse> => {
    try {
      ...
      console.error('생산지시 정보 조회 실패:', error);
      return {
        success: false,
        error: error instanceof Error ? error.message : '생산지시 정보 조회에 실패했습니다.'
      };
    }
  };
  
  // 생산지시 정보 단건 조회 (GET /work-orders/:id)
