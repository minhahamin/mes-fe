...
const ProductionPlanInfo: React.FC = () => {
  // 상태 관리
  const [productionData, setProductionData] = useState<ProductionPlanData[]>(INITIAL_DATA);
  ...
  const handleSave = useCallback(() => {
    if (!formData.planId || !formData.productCode || !formData.productName) {
    ...
    if (editingId) {
      // 수정
      const updatedItem = { ...formData, updatedAt: new Date().toISOString().split('T')[0] } as ProductionPlanData;
  ...
  const handleInputChange = useCallback((e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    setFormData(prev => ({ 
      ...prev, 
      [name]: ['planQuantity', 'estimatedHours', 'actualHours'].includes(name) ? Number(value) : value 
    }));
  }, []);
